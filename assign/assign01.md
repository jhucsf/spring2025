...
```bash
curl -O https://jhucsf.github.io/spring2025/assign/csf_assign01.zip
```
...
<div class='admonition caution'>
  <div class='title'>Important restriction</div>
  <div class='content' markdown='1'>
In your implementation of the functions,
...
The `uint256_lshift` function should shift all of the bits of a given
`UInt256` value left by the specified number of bits. As with the
left-shift operator (`<<`) for the built-in unsigned integer types,
the bits shifted out on the left disappear, and bits with the value
0 are shifted in on the right. (Note that by "left" we mean the most-significant
part of the bitstring, and by "right" we mean the least-significant
part of the bitstring.)
...
Note that when shifting a `uint32_t` value (in either direction), shifting by 32 or more
positions is undefined behavior.  Also note that left-shifting a bit
with the value 1 into or past the sign bit of a 32-bit value is
undefined behavior. It is a good idea to avoid doing bitwise operations
on signed values.
...
You should find the functions `uint256_add` and `uint256_add` useful for
implementing multiplication. (And, you'll want to implement and test them
before working on multiplication.)
...
(gdb) run test_add_2
Starting program: /home/daveho/git/csf-spring2025-private/src/csf_assign01_solution/uint256_tests test_add_2
test_add_2...
